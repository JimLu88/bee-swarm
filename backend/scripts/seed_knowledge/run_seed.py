...
def _load_libs() -> dict[str, dict]:
    ...
    try:
        from .lib_tax_insurance import LIBRARIES as ti
        libs["tax_insurance"] = ti
    except Exception as e:
        print(f"[warn] tax_insurance 书库加载失败: {e!r}")
    try:
        from .lib_learning_planning import LIBRARIES as lp
        libs["learning_planning"] = lp
    except Exception as e:
        print(f"[warn] learning_planning 书库加载失败: {e!r}")
    try:
        from .lib_travel_planning import LIBRARIES as tp
        libs["travel_planning"] = tp
    except Exception as e:
        print(f"[warn] travel_planning 书库加载失败: {e!r}")
    try:
        from .lib_dining_recommendation import LIBRARIES as dr
        libs["dining_recommendation"] = dr
    except Exception as e:
        print(f"[warn] dining_recommendation 书库加载失败: {e!r}")
    try:
        from .lib_purchase_decision import LIBRARIES as pd
